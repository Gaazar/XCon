...
namespace Sharp
{
	...
	private:
		struct StringView
		{
			...
			~StringView()
			{
				if (EmptyString().strview == nullptr || str != EmptyString().strview->str)
					free(str);
			}
		};
	...
	public:
		...
		virtual int64_t Read(char* buffer, int64_t len) = 0;
		virtual int64_t Write(char* buffer, int64_t len) = 0;
	...
	public:
		...
		long Read(char* buffer, long len)
		{
			long size = len;
		...
		long Write(char* buffer, long len)
		{
	...
	public:
		...
		int64_t Write(char* buffer, int64_t len);
		void Seek(int64_t offset, SeekOrigin origin);
		void Flush();
		int64_t Position();
		void Position(int64_t pos);
	...
	public:
		...
		bool Read(T& value)
		{
			auto sz = stream->Read(reinterpret_cast<char*>(&value), sizeof T);
			return sz == sizeof T;
		}
	...
	public:
		...
		bool Write(T& value)
		{
			auto sz = stream->Write(reinterpret_cast<char*>(&value), sizeof T);
			return sz == sizeof T;
		}
		//length prepend utf-8 encoding
